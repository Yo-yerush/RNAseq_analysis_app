...
if (!exists("is_remote_path", mode = "function")) {
  is_remote_path <- function(path) grepl("^(https?|ftp)://", path, ignore.case = TRUE)
}
if (!exists("DEFAULT_ARABIDOPSIS_DESCRIPTION_URL")) {
...
load_te_gene_map <- function(description_file) {
  if (!is_remote_path(description_file) && !file.exists(description_file)) {
    stop("Description file not found: ", description_file)
  }
  desc <- if (exists("read_any_table", mode = "function")) {
    read_any_table(description_file)
  } else {
    read.csv(description_file, check.names = FALSE)
  }
...
load_te_file <- function(te_file) {
  if (!is_remote_path(te_file) && !file.exists(te_file)) {
    stop("TAIR10 TE file not found: ", te_file)
  }
  te <- if (exists("read_any_table", mode = "function")) {
    read_any_table(te_file, delim = "\t")
  } else {
    read.csv(te_file, sep = "\t", check.names = FALSE)
  }
...
build_te_overlap_table <- function(de_df,
                                   description_file = DEFAULT_ARABIDOPSIS_DESCRIPTION_URL,
...
plot_retro_te_volcano_data <- function(retro_df,
                                       color_col = "Transposon_Super_Family",
                                       label_name = "Transposon\nSuper-Family",
                                       padj_cutoff = 0.05,
                                       lfc_cutoff = 1,
