...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(width = 3,
      ...
      conditionalPanel("input.tabs == 'Gene groups (At)'",
      ...
      conditionalPanel("input.tabs == 'PMN (plants)'",
        sliderInput("pmn_plot_width",  "Plot width (px)",  min = 200, max = 1600, value = 650, step = 50),
        sliderInput("pmn_plot_height", "Plot height (px)", min = 200, max = 1200, value = 400, step = 50)
      ),
          ...
          fluidRow(
            ...
            column(4,
              wellPanel(
                ...
                selectInput("go_keytype", "Gene ID type", choices = c("TAIR", "ENTREZID", "SYMBOL", "ENSEMBL"), selected = "TAIR"),
          ...
          tabsetPanel(
            tabPanel("Enrichment Analysis",
              wellPanel(
                fluidRow(
                  ...
                  column(5, div(class = "muted", "Uses the organism selected in the Organism annotations tab. The KEGG code is auto-filled when available."))
                )
              ),
              fluidRow(
                column(12,
            ...
            tabPanel("Pathview Visualization",
              wellPanel(
                ...
                tags$hr(),
                tags$label("Pathview colors", style = "font-weight: 600; font-size: 0.95em;"),
                fluidRow(
                  column(4,
                    tags$div(style = "display: flex; align-items: center; gap: 8px; margin-top: 6px;",
                      tags$input(type = "color", id = "pathview_color_up", value = "#B2182B",
                        style = "width: 42px; height: 30px; border: none; cursor: pointer; padding: 0;",
                        oninput = "Shiny.setInputValue('pathview_color_up', this.value, {priority: 'event'})"),
                      tags$span("Upregulated", style = "font-size: 0.9em;")
                    )
                  ),
                  column(4,
                    tags$div(style = "display: flex; align-items: center; gap: 8px; margin-top: 6px;",
                      tags$input(type = "color", id = "pathview_color_down", value = "#2166AC",
                        style = "width: 42px; height: 30px; border: none; cursor: pointer; padding: 0;",
                        oninput = "Shiny.setInputValue('pathview_color_down', this.value, {priority: 'event'})"),
                      tags$span("Downregulated", style = "font-size: 0.9em;")
                    )
                  ),
                  column(4,
                    tags$div(style = "display: flex; align-items: center; gap: 8px; margin-top: 6px;",
                      tags$input(type = "color", id = "pathview_color_mid", value = "#B3B3B3",
                        style = "width: 42px; height: 30px; border: none; cursor: pointer; padding: 0;",
                        oninput = "Shiny.setInputValue('pathview_color_mid', this.value, {priority: 'event'})"),
                      tags$span("Near zero", style = "font-size: 0.9em;")
                    )
                  )
                ),
        ...
        tabPanel("Log / Help",
          tabsetPanel(
            ...
            tabPanel("Help",
              ...
              h4("Notes & Usage"),
              tags$ul(
                tags$li(strong("Data input: "), "Upload DE CSV/TSV/TXT files or run DESeq2 directly from RSEM ", code("*.genes.results"), " files. CSV, TSV, and TXT uploads can use comma or tab delimiters."),
                ...
                tags$li(strong("DE plots: "), "Volcano and MA plots use ", code("gene_id"), ", ", code("log2FoldChange"), ", ", code("padj"), " and optional ", code("baseMean"), ". PCA is available only after running DESeq2 from RSEM count data."),
                ...
                tags$li(strong("DE results: "), "Table of the DE analysis results, including genes annotations (if available). Searches all columns in the current annotated DE table. Use spaces for AND-style matching and ", code("|"), " for OR-style matching."),
                ...
                tags$li(strong("Gene groups: "), "Downloads curated gene-set lists on first use, then builds group-specific volcano plots and tables."),
...
server <- function(input, output, session) {
  ...
  apply_selected_taxon_to_analysis <- function(tax_id, organism_name = NULL) {
    ...
    if (!is.null(cfg)) {
      ...
      updateSelectInput(session, "go_keytype", selected = cfg$go_keytype)
  ...
  default_annotation_label_for_tax <- function(tax_id) {
    tax_id <- suppressWarnings(as.integer(tax_id))
    if (identical(tax_id, 3702L)) return("Default Arabidopsis description file (GitHub)")
    if (identical(tax_id, 9606L)) return("Default human description file (GitHub)")
    ...
    if (nzchar(selected_col)) {
      ...
      if (identical(selected_col, "condition")) {
        ...
        controls <- c(controls, list(
          ...
          div(class = "muted", "Without interaction, the model adjusts for this effect. With interaction, the contrast is calculated within the selected effect level.")
        ))
      }
    }
  ...
  loaded_de <- reactive({
    if (input$data_mode == "example") {
      ...
      standardize_de_table(read_any_table(example_csv), merge_default_description = FALSE)
    } else if (input$data_mode == "csv") {
      req(input$de_file$datapath)
      standardize_de_table(read_any_table(input$de_file$datapath), merge_default_description = FALSE)
    } else {
      rv$de_base
    }
  })
  ...
  observeEvent(input$run_deseq, {
    ...
    withProgress(message = "Running DESeq2", value = 0.1, {
      tryCatch({
        ...
        rv$deseq_all_comparisons <- res$all_comparisons
        clear_analysis_results()
  ...
  output$data_summary_box <- renderUI({
    ...
    if (!is.null(input$treatment) && nzchar(input$treatment %||% "") &&
      ...
      contrast_line <- paste0("condition: ", input$treatment, " vs ", input$control)
    }
    wellPanel(
  ...
  output$annotation_preview <- renderDT({
    ...
    datatable(head(d, 5000), rownames = FALSE, filter = "top", options = list(pageLength = 12, scrollX = TRUE))
  })
  ...
  observeEvent(input$go_orgdb, {
    ...
    if (nrow(cfg) > 0) updateSelectInput(session, "go_keytype", selected = cfg$go_keytype[1])
  ...
  output$pmn_pathway_codes_ui <- renderUI({
    ...
    ids <- if (!is.null(d) && nrow(d) > 0 && "pathway.code" %in% names(d)) {
      head(stats::na.omit(as.character(d$pathway.code)), 3)
    } else {
      "PWY-6477"
    }
    textAreaInput(
      ...
      rows = 2,
      value = paste(ids, collapse = ", "),
      placeholder = "Example: PWY-6477. Separate multiple codes with comma, semicolon, or space."
    )
  })
  ...
  observeEvent(input$run_pmn_pathway_lookup, {
    ...
    withProgress(message = "Looking up PMN pathway genes", value = 0.2, {
      tryCatch({
        ...
        append_log("PMN pathway lookup completed:", nrow(rv$pmn_pathway_lookup), "pathway rows.")
      }, error = function(e) {
        rv$pmn_pathway_lookup <- NULL
  ...
  output$pmn_pathway_table <- renderDT({
    req(rv$pmn_pathway_lookup)
    datatable(rv$pmn_pathway_lookup, rownames = FALSE, filter = "top", options = list(pageLength = 10, scrollX = TRUE))
  })
  ...
  output$download_pmn_pathway_table <- downloadHandler(
    ...
    content = function(file) {
      req(rv$pmn_pathway_lookup)
      write.csv(rv$pmn_pathway_lookup, file, row.names = FALSE)
    }
  )
    ...
    withProgress(message = progress_message, value = 0.5, {
      tryCatch({
        ...
        pv_res <- pathview(
          ...
          low = list(gene = input$pathview_color_down %||% "#2166AC", cpds = input$pathview_color_down %||% "#2166AC"),
          mid = list(gene = input$pathview_color_mid %||% "#B3B3B3", cpds = input$pathview_color_mid %||% "#B3B3B3"),
          high = list(gene = input$pathview_color_up %||% "#B2182B", cpds = input$pathview_color_up %||% "#B2182B"),
          kegg.native = TRUE,
          gene.idtype = "KEGG"
        )
  ...
  observeEvent(list(input$pathview_color_up, input$pathview_color_down, input$pathview_color_mid), {
    req(rv$de, rv$pathview_pwid)
    append_log("Updating Pathview colors for:", rv$pathview_pwid, level = "STEP")
    generate_pathview_map(rv$pathview_pwid, "Updating Pathview colors...")
  }, ignoreInit = TRUE)
