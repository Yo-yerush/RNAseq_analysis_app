...
server <- function(input, output, session) {
  ...
  output$search_annotations_table <- renderDT({
    ...
    search_column_defs <- list(
      list(
        ...
        render = JS(
          ...
          "  if (type === 'display' && data != null && typeof data === 'string' && data.indexOf('gene-counts-btn') !== -1) {",
          "    return data;",
          "  }",
    ...
    if (isTRUE(has_count_buttons)) {
      search_column_defs <- c(gene_count_button_defs, search_column_defs)
    }
    
    # Use JavaScript to truncate long text cells and show full text on hover
    datatable(d, rownames = FALSE, filter = "top", selection = "none", escape = !isTRUE(has_count_buttons),
      options = list(
      ...
      callback = if (isTRUE(has_count_buttons)) gene_count_button_callback else JS("")
    )
  })
