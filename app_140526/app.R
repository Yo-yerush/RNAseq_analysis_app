...
server <- function(input, output, session) {
  ...
  output$gene_counts_click_hint <- renderUI({
    ...
    div(class = "muted", "Click a gene row to view normalized counts for the selected treatment/control comparison.")
  })
  ...
  observeEvent(input$search_annotations_table_rows_selected, {
    req(input$tabs == "DE results")
    if (!identical(input$data_mode, "rsem") || is.null(rv$norm_counts) || is.null(rv$coldata)) return()
    selected_row <- input$search_annotations_table_rows_selected[1]
    if (is.na(selected_row) || selected_row < 1 || selected_row > nrow(rv$de)) return()
    gene_id <- rv$de$gene_id[selected_row]
  ...
  output$pathview_table <- renderDT({
    ...
    datatable(rv$pathview_table, rownames = FALSE, filter = "top", options = list(pageLength = 15, scrollX = TRUE))
  })
