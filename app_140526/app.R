        ...
        tabPanel("Genes groups",
          tabsetPanel(
            tabPanel("Gene families - enrichment",
              wellPanel(
                fluidRow(
                  column(5, uiOutput("gene_family_enrichment_status_ui")),
                  column(4, selectInput("gene_family_enrichment_direction", "Gene set", choices = c("up", "down", "all"), selected = "up")),
                  column(3, numericInput("gene_family_min_size", "Min genes per family", value = 3, min = 1, step = 1)),
                  column(12, uiOutput("gene_family_enrichment_run_ui"))
                )
              ),
              fluidRow(
                column(12,
...
server <- function(input, output, session) {
  ...
  output$gene_family_enrichment_run_ui <- renderUI({
    ...
    actionButton("run_gene_family_enrichment", "Run family enrichment", class = "btn-success", style = "width:100%;")
  })
  ...
  output$gene_family_enrichment_plot <- renderPlot({
    p <- tryCatch(gene_family_enrichment_plot_reactive(), error = function(e) NULL)
    if (is.null(p)) {
      plot.new(); text(0.5, 0.5, "Run family enrichment, or no significant families passed the cutoff.", cex = 1.1)
    } else p
  }, width = function() input$grp_plot_width, height = function() input$grp_plot_height)
