            ...
            tabPanel("REVIGO-like reduction",
              wellPanel(
                ...
                fluidRow(
                  ...
                  column(3, br(), actionButton("run_revigo", "Run REVIGO-like analysis", class = "btn-primary", style = "width:100%;"))
                )
              ),
              fluidRow(
                column(12,
                  h4(textOutput("revigo_title", inline = TRUE)),
                  plotOutput("revigo_plot", width = "auto", height = "auto"),
                  download_plot_ui("revigo", "Download REVIGO plot"),
...
server <- function(input, output, session) {
  ...
  output$revigo_plot <- renderPlot({
    if (is.null(rv$revigo)) { plot.new(); text(0.5, 0.5, "Select a gene set and run REVIGO-like analysis.", cex = 1.1) } else rv$revigo$plot
  }, width = function() input$go_plot_width, height = function() input$go_plot_height)
  # ---- TE Analysis -------------------------------------------------------
